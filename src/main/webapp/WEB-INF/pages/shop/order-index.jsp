    ...
    <template>
            ...
            <template v-if="!isVirtual">
                ...
                <section class="address">
                    ...
                    <div class="content">
                        ...
                        <div class="btn_wrap">
                            <a href="javascript:void(0)" @click="isShowAddressList=true;isGroupBuyAddress=false;" class="f_left"
                               v-if="selectedDeliver || addressList">
                                <span>切换地址</span>
                            </a>
                            <a href="javascript:void(0)" @click="isGroupBuyAddress=false;editAddress(0)" class="f_right">
                                <span>添加地址</span>
                            </a>
                        </div>
                    </div>
                ...
                <section class="delivery_time" v-if="isNeedDeliveryDate">
                    <h2>
                        ...
                        <span v-if="!editingTime.isEdit && editingTime.hasSet" @click="editingTime.isEdit=true">[修改]</span>
                        <span v-if="editingTime.isEdit && editingTime.hasSet" @click="editingTime.isEdit=false">[关闭]</span>
                    </h2>
                    <div class="content">
                ...
                <ul>
                    <li class="p_item" v-for="item of goodsList" v-if="!item.isGroupBuyGoods">
                        <div class="col_wrap">
                            ...
                            <div class="col col_2 birthday_card">{{item.attributes && item.attributes.goods_birthday || '无'}}</div>
        ...
        <section class="enjoy_balance">
            ...
            <div class="content">
                <template v-if="orderInfo.user.balance.cashback>0">
                    ...
                    本次使用
                    <input type="number" maxlength="11" v-model.number.trim="enjoyBalance.value" v-if="enjoyBalance.isEdit">
        ...
        <section class="deliver_right" v-if="isHasDeliverRight">
            ...
            <h2 class="no_after hide">
                <strong>运费券</strong>
                <span class="gray">（可使用次数：1次）<i class="desc_btn" @click="isShowDeliverRightDesc=true">权益说明</i></span>
                <label @click="toggleDeliverRight">
                    <input type="checkbox" class="ios_checkbox" :checked="isUseDeliverRight" id="toggleDeliverRight"/>
                </label>
            </h2>
        ...
        <section class="preferential"
            ...
            <div class="detail_wrap"
                ...
                <div class="coupon_list" v-show="showCouponList">
                    ...
                    <ul v-if="couponList.length" class="clear_fix coupon_type">
                        <li v-for="(item,index) of couponList" @click="useCoupon(item.coupon_code)" :key="index"
                            :class="{active:selectedCouponCode===item.coupon_code}" v-show="isShowAllCoupon || index<4">
                            <div class="price">
                ...
                <div class="gold_card_list" v-show="showGoldCardList">
                    ...
                    <div class="tips">
                        <label :class="{active:isAgreeGoldCardRule}" @click="isAgreeGoldCardRule=!isAgreeGoldCardRule">true/false</label>
        ...
        <section class="toast_one" v-if="toastOneInfo.bargain_id">
            ...
            <div class="content">
                <div class="toast_info clear_fix">
                    ...
                    <div class="img_wrap">
                        <img :src="toastOneInfo.main_pic|mainPic">
                    </div>
                    <div class="detail">
            ...
            <div class="toast_slide" id="toastSlide" @click="jumpToToastOne">
                <img src="//newimgcdn.lapetit.cn/web/images/shopping/toastSlide.png">
            </div>
        </section>
        <!--金额信息-->
                ...
                <div class="form">
                    ...
                    <template v-if="!isExchange">
                        ...
                        <dl v-if="!isVirtual">
                            ...
                            <h5 class="orange" v-if="checkoutConfig.customText && checkoutConfig.customText.billsubmit">
                    ...
                    <template v-if="isExchange">
                        <dl v-if="deliveryDate">
                            <dt>熊抱吐司商品开始配送时间：</dt>
                            <dd>
                                {{deliveryDate}} <span>{{startTime|parseTime}}:00-{{endTime|parseTime}}:00；</span>
                            </dd>
                        </dl>
            ...
            <div class="inner">
                ...
                <div class="form">
                    ...
                    <div class="edit_item detail_wrap clear_fix" @mouseleave.self="searchList=[]">
                        ...
                        <div class="input_wrap need_tips">
                            <div class="search_wrap">
                                <input type="text" v-model.trim="address.address" maxlength="50" @input="search">
                                <div class="search_list" v-show="searchList.length">
                                    <ul>
                                        <li v-for="item of searchList" @click="setDetail(item.name,item.county)">
                    ...
                    <div class="edit_item clear_fix room_no_item">
                        <label>门牌号</label>
                        <input type="text" placeholder="门牌号  如1栋101" v-model.trim="address.roomNo" maxlength="30">
                    </div>
                    <div class="edit_item clear_fix">
                        <label>收货人</label>
                        <div class="input_wrap">
                            <input type="text" placeholder="收货人姓名" v-model.trim="address.custName" maxlength="20">
                        </div>
                    </div>
                    <div class="edit_item clear_fix">
                        <label>手机</label>
                        <div class="input_wrap">
                            <input type="tel" placeholder="收货人手机号码" v-model.trim="address.mobile" maxlength="11">
                            <span>或 固定电话</span>
                            <input type="text" placeholder="收货人电话号码" v-model.trim="address.phone" maxlength="20">
                        </div>
                    </div>
                    <div class="btn_wrap">
                        <a href="javascript:void(0)"
                        ...
                        <button @click="resetAddressDetail" class="cancel_btn" v-show="address.deliverID!==0">取消</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="value_card_check pop_view" v-show="isShowValueCardCheck">
            ...
            <div class="inner">
                ...
                <div class="form">
                    ...
                    <p class="input_wrap">
                        <label>手机号</label>
                        <input type="tel" maxlength="11" class="mobile" :value="valueCardInfo.showMobile" disabled/>
                    </p>
                    <p class="input_wrap yzm_wrap">
                        <label>验证码</label>
                        <input type="text" maxlength="4" v-model.trim="valueCardInfo.yzm"/>
                        <img data-src="/customer/captcha.html?width=90&height=40&offset=6&v=1"
                    ...
                    <p class="input_wrap code_wrap">
                        ...
                        <button class="btn" disabled v-show=" valueCardInfo.time > 0 ">{{valueCardInfo.time}}秒</button>
                    </p>
...
<script>
    ...
    var addressList = "";
    //百度关联搜索
    var searchUrl = 'https://lbsyun.lecake.com/suggestion?openargs=eyJnX3BsYXRmb3JtIjoibGVjYWtlIiwiZ19hZ2VudCI6Ikt5SFRvcU1SSzVmcSIsImdfY2xpY2siOiJLeWR5UnY5ZnB1N3MiLCJnX2FjY291bnQiOiI3NTMxNzAzIiwiZ190aW1lIjoxNTQyMDk0NDU1LCJnX3NpZ24iOiIzMmQyMzExYzk0MjU1ZWFmMGM2YTMyZmVhZjgwZGQxZCJ9';
    //结算页配置
    //结算页配置
    var checkoutConfig = {
        "buy_id": "Kych0LjCc-51gLvx",
        ...
        "site_id": 287,
        "haveValidOrder": null,
        "thirdText": "（该地区暂不支持货到付款）订购后七天内发货（法定节假日除外）",
        "customText": [],
        "activity": "",
        "supportList": {
            "393": "蛋糕"
        ...
        "payList": [{
            ...
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f4c21a34.jpg"
        }, {
            "payTypeID": "1",
            "payTypeName": "货到付款",
            "typeID": "1",
            "credit_cash": "2",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f5e0f245.jpg"
        }, {
            "payTypeID": "8",
            "payTypeName": "支付宝",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8fb95d0e1.jpg"
        }, {
            "payTypeID": "72",
            "payTypeName": "货到刷银行卡",
            "typeID": "1",
            "credit_cash": "2",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea986bced32.jpg"
        }, {
            "payTypeID": "96",
            "payTypeName": "银联支付",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f75ca8b5.jpg"
        }, {
            "payTypeID": "9",
            "payTypeName": "招商银行",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f89789bd.jpg"
        }, {
            "payTypeID": "30",
            "payTypeName": "建设银行",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8dd697cb4.jpg"
        }, {
            "payTypeID": "37",
            "payTypeName": "货到刷农行卡",
            "typeID": "1",
            "credit_cash": "2",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea957d4020d.jpg"
        }, {
            "payTypeID": "22",
            "payTypeName": "货到刷招行卡",
            "typeID": "1",
            "credit_cash": "2",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f9d6bdc1.jpg"
        }, {
            "payTypeID": "53",
            "payTypeName": "农业银行",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8e46667ac.jpg"
        }, {
            "payTypeID": "49",
            "payTypeName": "浦发银行",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8ed86c688.jpg"
        }, {
            "payTypeID": "61",
            "payTypeName": "广发银行",
            "typeID": "6",
            "credit_cash": "0",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8dc23beae.jpg"
        }, {
            "payTypeID": "63",
            "payTypeName": "货到刷平安银行信用卡",
            "typeID": "1",
            "credit_cash": "1",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8e830441a.jpg"
        }, {
            "payTypeID": "86",
            "payTypeName": "货到平安银行卡",
            "typeID": "1",
            "credit_cash": "2",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea915b0f314.jpg"
        }, {
            "payTypeID": "94",
            "payTypeName": "货到刷上海银行信用卡",
            "typeID": "1",
            "credit_cash": "1",
            "berfor_image": "payUpload\/2017\/04\/10\/58ea8f27dac24.jpg"
        }],
        "payment_id": "29",
        "getLastInvoice": false
    };
    //是否存在购买心享卡权益的虚拟商品
