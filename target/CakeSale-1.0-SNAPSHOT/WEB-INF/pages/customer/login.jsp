...
<head>
    ...
    <script src="<%=request.getContextPath()%>/static/js/identifyCode.js"></script>
...
<article class="container main_login main_passport max_width">
    ...
    <header class="global_header mini_header">
        ...
        <div class="main" style="background:url(<%=request.getContextPath()%>/static/images/ac198f1e37025872d33159b4cfc2ae88.jpg) no-repeat center;">
        ...
        <article class="login_app" id="loginApp">
            <template>
                <div class="login_wrap" v-if="currentForm==='login'">
                  ...
                  <form id="form1" action="<%=request.getContextPath()%>/login.html" method="post">
                    <div class="tab tab_mobile" v-show="login.loginType==='mobile'" >
                   <input type="hidden" id="num0" name="num" value="0" >
                        ...
                        <p class="input_wrap yzm_wrap">
                            <input type="text"
                                   ...
                                   id="yanzhengma"
                            />
                            <a href="" class="captchaWrap" >
                                <%--******验证码图片--%>
                                <img src="/identify/code.do" onclick="myRefersh(this)" id="yzm"/>
                                <span class="refresh_btn" >换一个</span>
                                    <span id="error_yzm" ></span>
                            </a>
                        ...
                        <p class="input_wrap yzm_wrap">
                           ...
                           <%-- <button onclick="login()" class="btn" >{{login.yzmLogin===1?'立即登录':'立即绑定'}}
                            </button>--%>
                        <button type="submit" class="btn">{{login.yzmLogin===1?'立即登录':'立即绑定'}}</button>
                    </div>
                  </form>
                    <form action="<%=request.getContextPath()%>/login.html" method="post" id="form2">
                    <div class="tab tab_account" v-show="login.loginType==='username'"  >
                  <input type="hidden" id="num1" value="1" name="num">
                        ...
                        <%--<p class="input_wrap yzm_wrap" :class="{hide:!login.isNeedCheckYzm}">
                            ...
                            <a href="javascript:void(0)" class="captchaWrap" @click="getCaptcha">
                                &lt;%&ndash;******验证码图片&ndash;%&gt;
                                &lt;%&ndash;<img :src="<%=request.getContextPath()%>/static/picture/login.captchaimg" alt="点击刷新">&ndash;%&gt;
                                <span class="refresh_btn">换一个</span>
                            </a>
                            <span class="error_message">{{login.yzmError}}</span>
                        </p>--%>
                            <%--<button onclick="login1()" class="btn" >{{login.yzmLogin===1?'立即登录':'立即绑定'}}
                            </button>--%>
                        <button type="submit" class="btn" >{{login.yzmLogin===1?'立即登录':'立即绑定'}}</button>
                    </div>
                    </form>
                    ...
                    <p class="clear_fix bottom_link" :style="{visibility:login.yzmLogin===1?'show':'hidden'}">
                        ...
                        <a href="javascript:void(0)" class="f_right link_2"
                           data-statistics="login_register|注册新用户" onclick="register()">立即注册</a>
                    ...
                    <div class="more_login_type" v-show="login.yzmLogin===1">
                        ...
                        <div class="type_list">
                            <a href="javascript:void(0)"
                               class="wx"
                               @click="loginByWx('/customer/union-login.html?return_url=https%253A%252F%252Fwww.lecake.com%252Fcustomer%252Findex.html&t=wx')"></a>
                            <a href="/customer/union-login.html?return_url=https%3A%2F%2Fwww.lecake.com%2Fcustomer%2Findex.html&t=qq"
                               class="qq"></a>
                        </div>
                    </div>
                </div>
                ...
                <div class="register_wrap" v-if="currentForm==='register'">
                    ...
                    <div class="tab">
                       ...
                       <%-- <p class="input_wrap yzm_wrap" v-else>
                            ...
                            <a href="javascript:void(0)" class="captchaWrap" @click="getCaptcha">
                                &lt;%&ndash;******验证码图片&ndash;%&gt;
                                &lt;%&ndash;<img :src="<%=request.getContextPath()%>/static/picture/register.captchaimg" alt="点击刷新">&ndash;%&gt;
                                <span class="refresh_btn">换一个</span>
                            </a>
                        ...
                        <p class="input_wrap yzm_wrap">
                            <input type="text"
                                   ...
                                   name="registerCode"
                            />
                            <button class="code_btn" onclick="sendMessage1()" v-show=" !register.isCodeGetting && register.time == 0 ">
                                发送验证码
                            </button>
                    ...
                    <button onclick="register()" class="btn" :disabled="register.isFetching">立即注册</button>
                ...
                <div class="bind_wrap" v-if="isShowBind">
                    ...
                    <div class="inner">
                        ...
                        <p class="input_wrap yzm_wrap">
                            ...
                            />
                            <button class="code_btn" onclick="sendMessage1()" v-show=" !bind.isCodeGetting && bind.time == 0 ">发送验证码
                            </button>
...
<script>
    var lastUrl = 'https://www.lecake.com/customer/index.html';
...
<script type="text/javascript">var glbStatConfig={stat_host:"www.lecake.com",click_id: "KyJ6mAk7stob"};</script>
...
<script type="text/javascript">
    ...
    function sendMessage1() {
        ...
        var jbPhone =$("#phoneNum1").val(); //手机号码
...
//            }
//        });
        window.location.href="<%=request.getContextPath()%>/customer/register.html";
    }
    /*function login() {
        var num0=$("#num0").val();//0为手机号登陆
        if(num0=='0'){

            var codeMobile = $("#codeMobile").val(); //验证码
            var phoneNum =$("#phoneNum").val(); //手机号码
            var yanzhengma=$("#yanzhengma").val();
            var yzm=$("#yzm").val();
            if(!yanzhengma==yzm){
                $("#error_yzm").html("验证码错误")
            }
            $("#form1").submit();
            $.ajax({
                url : "/login.html",
                ...
                success : function(data) {
                    ...
                    if(data==1){
                        ...
                        $("#SmsCheckCodeTip").html("你的手机号没有注册");
                    }
   ...
   /* function login1(){//用户名密码登陆

            var num1=$("#num1").val();//1为用户名登陆
            var account=$("#usernamephoneNum").val();
            var password=$("#password").val();
            $.ajax({
                url : "/login.html",
                data : {
